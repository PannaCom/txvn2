...
@property (nonatomic) NSMutableArray *mutableMenuItemCenterX;
...
- (void)initValues
{
    ...
    _mutableMenuItemCenterX = [NSMutableArray array];
...
- (void)configureUserInterface
{
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            CGRect itemWidthRect = [titleText boundingRectWithSize:CGSizeMake(WIDTH_SCREEN/2, WIDTH_SCREEN/2) options: NSStringDrawingUsesLineFragmentOrigin attributes: @{NSFontAttributeName:_menuItemFont} context: nil];
            ...
            if (i == 0) {
                [_mutableMenuItemCenterX addObject:@(_menuItemWidth/2+_menuMargin)];
            }
            else{
                [_mutableMenuItemCenterX addObject:@([_mutableMenuItemCenterX[i-1] floatValue]+[_mutableMenuItemWidths[i-1] floatValue]/2 + _menuItemWidth/2+_menuMargin)];
            }
...
//                        offset.x = _controllerScrollView.contentOffset.x * ratio;
                        offset.x = MIN(MAX(0, [_mutableMenuItemCenterX[(int)(_controllerScrollView.contentOffset.x/WIDTH_SCREEN)] floatValue]-WIDTH_SCREEN/2), _menuScrollView.contentSize.width - WIDTH_SCREEN);
                        if (offset.x > _menuScrollView.contentSize.width - WIDTH_SCREEN) {
                            offset.x = _menuScrollView.contentSize.width - WIDTH_SCREEN;
                        }
